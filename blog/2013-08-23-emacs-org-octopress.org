...
Emacs + org-mode 寫的，之後在透過插件 [[https://github.com/yoshinari-nomura/org-octopress][org-octopress]] 轉給 Octopress。
...
* Octopress
[[http://octopress.org/][Octopress]] 是一套 Blog 的框架，所以一開始你的 Blog 就有一個好看的外觀基礎，功能上
...
1. 開啟 Emacs 鍵入 =M-x org-octopress RET= ，會到 org-octopress 的界面鍵入 =w=
   和標題，開始撰寫文章。
2. 寫完後，鍵入 =C-c C-e P x octopress=
3. 在你的 octopress 根目錄鍵入 =bundle exec rake gen_deploy=
4. 還有 source 也要 push 上去 =git push -u origin source=

文章打到一半或要做確認的時候用 =bundle exec rake preview= ，之後在
http://localhost:4000/ 可以看你的 Blog。

順帶一提要 push 兩次還真累，還在找辦法解決。
